...
def _print_plan_diagnostics(action_command):
    ...
    if plan_data:
        ...
        print(f"🧠 [VLM 요약] target={vlm_target}, bbox={plan_data.get('bbox_norm')}")

    motor_cmds = action_command.get("motor_commands", [])
    print(f"🤖 [Step2] action={action_command.get('action')}, motor={motor_cmds}")
