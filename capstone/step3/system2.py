...
class VisionPlanner:
    ...
    def build_motor_prompt(self, target_class, target_distance_m, target_yaw_deg, target_aligned,
                           obstacles_info):
        if obstacles_info:
            obs_lines = [
                f"- {o['class']}: distance={o['distance']:.2f}m, yaw={o['yaw_deg']:.2f}deg, bbox_width={o['bbox_width_px']}px"
                for o in obstacles_info
            ]
            obs_str = "OBSTACLES IN PATH:\n" + "\n".join(obs_lines)
        else:
            obs_str = "OBSTACLES: none"

        return f"""You are a motor control module for a home service robot.
Given sensor measurements, output a single motor command to approach the target while avoiding obstacles.

TARGET: {target_class}
DISTANCE: {target_distance_m:.2f}m
YAW_TO_TARGET: {target_yaw_deg:.2f}deg (negative=left, positive=right)
ALIGNED: {target_aligned}

{obs_str}

RULES:
- If distance to target <= 0.3m and no obstacle is dangerously close: output "STOP"
- If an obstacle is close (< 0.5m) and blocks the path between robot and target: TURN AWAY from obstacle
  - obstacle yaw negative (on left) -> TURN RIGHT to avoid
  - obstacle yaw positive (on right) -> TURN LEFT to avoid
- If not aligned with target (ALIGNED=False) and no close obstacle: TURN toward target
- If aligned and path is clear: MOVE FRONT

Turn angle: 5 to 30 degrees
Move distance: 0.05 to 0.40 meters

Output a single motor command. Decide the next action based on current snapshot only.
Multi-step sequences (e.g. avoid-then-return) will be reconstructed across multiple cycles.
...
-> {{"motor_command": "TURN RIGHT 20.00deg"}}
"""
