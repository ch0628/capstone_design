...
class VisionPlanner:
    def __init__(self):
        ...
        self.obstacle_assumed_distance_m = 1.0  # depth 연결 전 임시
...
"""
    ...
    def plan(self, image_path, command, current_distance, avoidance_attempts=0):
        """
        이미지+명령 → action_command.

        Args:
            image_path: 카메라 이미지 경로
            command: 자연어 사용자 명령
            current_distance: target까지 현재 거리(m). 외부에서 제공.
        ...
        try:
            ...
            print(
                f"\n📸 [System2] '{image_path}' "
                f"(명령: {command}, distance={current_distance:.2f}m, "
                f"attempts={avoidance_attempts})"
            )
            ...
            if plan is None:
                return {"status": "abort", "action": "abort", "reason": "vlm_parse_error"}
            ...
            for od in obstacle_dets:
                ...
                obs_distance = self.obstacle_assumed_distance_m

                blocking, reason = self.is_blocking_obstacle(
                    og, obs_distance, target_geom, current_distance
                )

                entry = {
                    "class": od["class"],
                    "conf": round(od["conf"], 3),
                    "bbox": od["bbox"],  # raw bbox 그대로 (System 1이 회피각 계산용)
                    "yaw_deg": og["yaw_deg"],
            ...
            target_info = {
                ...
                "distance_error": current_distance - self.target_distance_m,
            }
            ...
            return {"status": "abort", "action": "abort", "reason": str(e)}