...
class VisionPlanner:
    def __init__(self):
        ...
        # 시스템 설정
        self.target_distance_m = 0.2
        self.emergency_brake_distance_m = 0.2

        print("✅ System2(VisionPlanner) 준비 완료 (20cm_vlm)")
...
4. Estimate the distance to the target_object in meters.
...
- reasoning: A very short one-sentence explanation of why this object was chosen based on the user's state.
- estimated_distance_m: A float value representing the estimated distance to the target_object in meters.
- obstacle_classes: Objects between the robot and the target.

Output format (ONE JSON object only):
{{"intent": "<verb>", "state_analysis": "<user's state>", "reasoning": "<short explanation>", "target_object": "<class or null>", "estimated_distance_m": <float>, "obstacle_classes": ["<class>", ...]}}
"""
    ...
    def plan(self, image, command, depth_map, avoidance_attempts=0):
        ...
        try:
            ...
            # depth가 유효하지 않을 때 VLM 추정치 사용
            if target_distance is None:
                target_distance = plan.get("estimated_distance_m")
                print(f"⚠️ Depth unavailable, using VLM estimate: {target_distance}")

            if target_distance is None:
                return {"status": "abort", "action": "abort", "reason": "depth_error", "timings": timings}
