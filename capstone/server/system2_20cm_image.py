...
class VisionPlanner:
    def __init__(self):
        ...
        print("✅ System2(VisionPlanner) 준비 완료 (20cm_image)")
...
"""
    ...
    def _estimate_distance_from_image(self, bbox, img_w, img_h):
        """
        이미지 바운딩 박스 크기를 기반으로 거리를 추정 (Depth 실패 시 대안).
        """
        pixel_h = bbox[3] - bbox[1]
        if pixel_h <= 0: return None
        
        # hfov = 70 deg, fx = fy 가정
        hfov_rad = math.radians(self.hfov_deg)
        f_pixel = (img_w / 2.0) / math.tan(hfov_rad / 2.0)
        
        # 일반적인 물체 높이를 15cm로 가정하여 거리 계산
        real_h = 0.15 
        dist = (f_pixel * real_h) / pixel_h
        return dist
    ...
    def plan(self, image, command, depth_map, avoidance_attempts=0):
        ...
        try:
            ...
            # depth가 유효하지 않을 때 이미지를 기반으로 거리 추정
            if target_distance is None:
                target_distance = self._estimate_distance_from_image(target_det["bbox"], img_w, img_h)
                print(f"⚠️ Depth unavailable, using image-based estimate: {target_distance:.2f}m")
