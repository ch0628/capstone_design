...
def send_one_shot(color_img, depth_img, command_to_send):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        sock.sendall(header + cmd_data + color_data)
...
def print_stats(res, t_start, t_end):
    ...
    next_hint = res.get('next_hint', 'N/A')
    reason = res.get('reason')
    motor_cmds = res.get('motor_commands', [])
    ...
    if status in ('abort', 'error', 'retry'):
        ...
        if motor_cmds:
            print(f"[모터]  {' → '.join(motor_cmds)}")

    print('-' * 60)
    print(f"  1. Pi → Server     {fmt(pi_to_server)}")
    print(f"  2. VLM 추론        {fmt(vlm_time)}")
    print(f"  3. Post-processing {fmt(post_time)}")
    print(f"     (Plan 합계      {fmt(plan_total)})")
    print(f"  4. Server → Pi    {fmt(server_to_pi)}")
    print('-' * 60)
    print(f"  5. 전체 RTT       {total_rtt:>7.2f} ms")
    print('=' * 60)
...
def run_one_cycle(node, motor, current_command, command_dirty):
    ...
    if motor_commands:
        motor.execute_motions(motor_commands)
    else:
        print('   (실행할 모터 명령 없음)')
...
def main():
    ...
    try:
        while True:
            ...
            last_hint = result.get('next_hint')

            if last_hint == 'retry':
                retry_count += 1
                reason = result.get('reason')
                print(f"   🔁 retry {retry_count}/{MAX_RETRY} (reason: {reason})")

                if retry_count >= MAX_RETRY:
                    print(f"\n⚠️  {MAX_RETRY}회 연속 retry 실패 ({reason}) → 사용자 대기")
                    last_hint = 'abort'
                    retry_count = 0
                else:
                    last_hint = None
